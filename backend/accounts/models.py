...
class OTPVerification(models.Model):
    """
    Stores OTPs for:
      - email_register   : verify email on signup
      - email_reset      : forgot password via email
      - whatsapp_register: verify phone via WhatsApp
      - whatsapp_reset   : reset password via WhatsApp OTP
    """
    OTP_TYPES = [
        ('email_register',    'Email Registration'),
        ('email_reset',       'Email Password Reset'),
        ('whatsapp_register', 'WhatsApp Registration'),
        ('whatsapp_reset',    'WhatsApp Password Reset'),
    ]

    user       = models.ForeignKey(User, on_delete=models.CASCADE,
                                   related_name='otps', null=True, blank=True)
    email      = models.EmailField(null=True, blank=True)   # used before user exists
